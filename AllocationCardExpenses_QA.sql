...
--STEP 3: Get total Card Fees Expenses by GL Account
...
,'Alloc-CardExpenses'+[G/LAccount] AS Account
...
JOIN #tableVar t 
	ON t1.[FiscalYearPeriod] = t.FiscalYearPeriodMA --FOR TESTING ONLY
WHERE t2.[PCA_HIEND] In ('PM202211', 'PM202212', 'PM202213', 'PM202221', 'PM202241' ) 
...
--STEP 4a: Calculate fuel volume ratio for each COT: step2 / step1 
...
--STEP 4b: Calculate Card Fees Expenses amount for each COT: step3 * step4a
...
SELECT t1.[FiscalYearPeriod]
	   ,t1.[Industry]
	   ,t2.Account
       ,t2.[Amount] * t1.[COTFuelRatio] AS COTCardExpenses
INTO #table5
FROM #table4 t1
JOIN #table3 t2
ON t1.[FiscalYearPeriod] = t2.[FiscalYearPeriod]
...
--STEP 5a.1 :get fuel volume for each outlet excl. Rtl Equity Prtnrship 
...
--STEP 5a.2: Get fuel volume ratio for each outlet excl. Rtl Equity Prtnrship: step5a.1 / step2
...
--STEP 5b: Calculate Card Fees Expenses amount for each outlet excl. Rtl Equity Prtnrship: step 5a.2 * step 4b
...
SELECT t1.[FiscalYearPeriod] 
	  ,t2.Account
	  ,t1.[Outlet]
	  ,CONVERT(DECIMAL(38,15),t1.OutletFuelRatio) * t2.[COTCardExpenses] AS Amount
INTO #table8
FROM #table7 t1
JOIN #table5 t2
ON t1.[FiscalYearPeriod] = t2.[FiscalYearPeriod]
...
SELECT t2.[FiscalYearPeriod]
	  ,t2.[Account]
	  ,t1.RtlPartnershipCt
	  ,CONVERT(DECIMAL(38,15),t2.COTCardExpenses) /t1.RtlPartnershipCt AS RtlPartnerCardExpenses
INTO #table11
